...
const setImages = (race) => {
    previewImages.value = race.target.files;
    form.images = race.target.files;
};

const submitForm = async () => {
    const result = await v$.value.$validate();
    if (!result) return;
    form.post(route("dashboard.race-media.store", { race: props.race.uuid }), {
        onSuccess: () => {
            Inertia.visit(
                route("dashboard.race-media.index", { race: props.race.uuid })
            );
        },
    });
};
